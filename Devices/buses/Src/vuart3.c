...
void VUART3_Trans_Handler(void)
{
    if((data) && (sent < length))
    {
        USART_SendData(USART3, data[sent]);
        sent += 1;
    }
    else
    {
...
static void uart_init(enum __dev_state state)
{
    ...
    if(hcomm == INVALID_HANDLE_VALUE)
    {
    	TRACE(TRACE_INFO, "UART3 open failed.");
    	return;
    }
    ...
    if(!SetupComm(hcomm, 2048, 2048))
    {
		...
		TRACE(TRACE_INFO, "UART3 set attributes failed.");
		return;
	}
    ...
    if(tcgetattr(fd, &options) != 0)
    {
        close(fd);
        fd = -1;
        TRACE(TRACE_INFO, "UART3 get attributes failed.");
        return;
    }
